...
function M.setup()
  ...
  local function stop_native_sim_once(session)
  ...
  dap.listeners.after.event_terminated["native_sim_stop"] = stop_native_sim_once
  dap.listeners.after.event_exited["native_sim_stop"] = stop_native_sim_once

  local function continue_or_native_sim()
    if dap.session() ~= nil then
      dap.continue()
      return
    end
    M.run_launch("Debug native_sim (Clang)")
  end

  local map = vim.keymap.set
  map("n", "<F5>", continue_or_native_sim, { desc = "Debug native_sim Clang / continue" })
  ...
  map("n", "<leader>dn", function()
    M.run_launch("Debug native_sim (Clang)")
  end, { desc = "Debug native_sim Clang" })

  vim.api.nvim_create_user_command("DebugNativeSimClang", function()
    M.run_launch("Debug native_sim (Clang)")
  end, {})
end
